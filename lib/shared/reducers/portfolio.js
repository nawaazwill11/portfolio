"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = exports.addWindowEvents = exports.windowScroll = exports.setSectionsOffsets = exports.initial_state = exports.SET_ACTIVE_NAV = exports.SET_SECTIONS_OFFSETS = exports.SET_INITIALIZED = void 0;

var _toolkit = require("@reduxjs/toolkit");

const SET_INITIALIZED = 'SET_INITIALIZED';
exports.SET_INITIALIZED = SET_INITIALIZED;
const SET_SECTIONS_OFFSETS = 'SET_SECTIONS_OFFSETS';
exports.SET_SECTIONS_OFFSETS = SET_SECTIONS_OFFSETS;
const SET_ACTIVE_NAV = 'SET_ACTIVE_NAV';
exports.SET_ACTIVE_NAV = SET_ACTIVE_NAV;
const initial_state = {
  active_nav: 'profile',
  section_offset: {},
  data: {},
  intialized: false
};
exports.initial_state = initial_state;
const portfolioSlice = (0, _toolkit.createSlice)({
  name: 'portfolio',
  initialState: initial_state,
  reducers: {
    [SET_INITIALIZED]: (state, action) => ({ ...state,
      initialized: action.payload
    }),
    [SET_ACTIVE_NAV]: (state, action) => ({ ...state,
      active_nav: action.payload
    }),
    [SET_SECTIONS_OFFSETS]: (state, action) => ({ ...state,
      section_offsets: action.payload
    })
  }
});
const {
  actions
} = portfolioSlice;

const setSectionsOffsets = () => dispatch => {
  const getOffset = selector => {
    try {
      const element = document.querySelector(selector);
      return element.offsetTop;
    } catch (error) {
      return 0;
    }
  };

  const offsets = {
    profile: getOffset('#profile'),
    experiences: getOffset('#experiences'),
    abilities: getOffset('#abilities'),
    projects: getOffset('#projects'),
    contact: getOffset('#contact')
  };
  dispatch(actions[SET_SECTIONS_OFFSETS](offsets));
  dispatch(actions[SET_INITIALIZED](true));
};

exports.setSectionsOffsets = setSectionsOffsets;

const windowScroll = (offsets, getActiveNav) => dispatch => {
  const {
    profile,
    experiences,
    abilities,
    projects,
    contact
  } = offsets;
  const currentOffsetY = window.scrollY;
  const action = actions[SET_ACTIVE_NAV];
  const active_nav = getActiveNav();

  if (currentOffsetY >= profile && currentOffsetY < experiences) {
    if (active_nav !== 'Profile') dispatch(action('Profile'));
  } else if (currentOffsetY >= experiences && currentOffsetY < abilities) {
    if (active_nav !== 'Experiences') dispatch(action('Experiences'));
  } else if (currentOffsetY >= abilities && currentOffsetY < projects) {
    if (active_nav !== 'Abilities') dispatch(action('Abilities'));
  } else if (currentOffsetY >= projects && currentOffsetY < contact) {
    if (active_nav !== 'Projects') dispatch(action('Projects'));
  } else if (currentOffsetY >= contact) {
    if (active_nav !== 'Contact') dispatch(action('Contact'));
  }
};

exports.windowScroll = windowScroll;

const addWindowEvents = (offsets, getActiveNav) => dispatch => {
  window.addEventListener('scroll', () => {
    dispatch(windowScroll(offsets, getActiveNav));
  });
  window.addEventListener('resize', () => {
    dispatch(setSectionsOffsets());
  });
};

exports.addWindowEvents = addWindowEvents;
var _default = portfolioSlice;
exports.default = _default;